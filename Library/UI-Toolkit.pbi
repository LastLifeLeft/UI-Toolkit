DeclareModule UITK
	...
	Enumeration 5000 ; Gadget attributes
		...
		CompilerIf Defined(EnableTimeline, #PB_Module)
			...
			#Attribute_TimeLine_HoverItem			; TimeLine gadget: line under the pointer, -1 for none (read only)
			#Attribute_TimeLine_Folded				; TimeLine item: sub-rows hidden; a line with none never folds (read/write)
			#Attribute_TimeLine_SubRows				; TimeLine item: bands the line grows by when unfolded (read only)
			#Attribute_TimeLine_BlockCount			; TimeLine item: blocks sitting on the line (read only)
			
			#Attribute_MediaBlock_Position			; media block: start, relative to its parent block when it has one (read/write)
			#Attribute_MediaBlock_AbsolutePosition	; media block: start on the timeline itself (read only)
			#Attribute_MediaBlock_Duration			; media block: length in time units (read/write)
			#Attribute_MediaBlock_Color				; media block: the strip along its top (read/write)
			#Attribute_MediaBlock_Icon				; media block: ImageID drawn before the label, 0 for none (read/write)
			#Attribute_MediaBlock_Container			; media block: can hold children, and so gives its line a sub-row (read/write)
			#Attribute_MediaBlock_Selected			; media block: part of the current selection (read/write)
			#Attribute_MediaBlock_Data				; media block: the caller's own pointer (read/write)
	...
	Enumeration #PB_EventType_FirstCustomValue; EventType
		...
		CompilerIf Defined(EnableTimeline, #PB_Module)
			#EventType_TimeLinePlayerMove		; TimeLine: the playhead moved - EventData is its new position
			#EventType_TimeLineBlockSelect		; TimeLine: the block selection changed
	...
	CompilerIf Defined(EnableTimeline, #PB_Module)
		...
		Declare.i AddMediaBlock(Gadget, Line, Position, Duration, Text.s, Color = 0, Icon = 0, *Data = 0, *Parent = 0)	; the new block, 0 on failure
		...
		Declare SetMediaBlockText(Gadget, *Block, Text.s)
		Declare.i CountMediaBlocks(Gadget, Line = -1)				; Line -1 counts every block the gadget holds, children included
		Declare.i GetMediaBlock(Gadget, Line, Index)
	...
	; ParameterList
	CompilerIf Defined(EnableParameterList, #PB_Module)
		Enumeration ; What a ParameterList row is - set with #Attribute_ParameterList_Kind
			#ParameterList_Value				; a name, an expression, and the reading it comes to
